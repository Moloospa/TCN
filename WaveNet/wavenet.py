...
def WaveNetResidualConv1D(num_filters, kernel_size, dilation_rate):
    ...
    def build_residual_block(l_input):
        # Gated activation.
        l_dilated_conv = Conv1D(num_filters, kernel_size, dilation_rate=dilation_rate,
            padding="causal", use_bias=False, kernel_initializer='he_normal',)(l_input)
        l_sigmoid = Activation('sigmoid')(l_dilated_conv)
        l_tanh = Activation('tanh')(l_dilated_conv)
        l_mul = Multiply()([l_sigmoid, l_tanh])
...
def build_wavenet_model(input_size, num_filters, kernel_size,
    ...
    l_stack_conv1d = Conv1D(num_filters, kernel_size, padding="causal", use_bias=False, kernel_initializer='he_normal',)(l_input)
    l_skip_connections = []
    ...
    l_output = Dense(1, activation="softmax")(l_flatten)
